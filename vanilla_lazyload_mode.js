...
/* const POSTER = "poster"; */
  ...
  const defaultSettings = {
	elements_selector: "IMG, VIDEO, IFRAME, IFRAMES",
...
/*	const statusesAfterLoading = [statusLoading, statusLoaded, statusApplied, statusError]; */
  ...
  const addTempImage = element => {
	element.AllTempImage = document.createElement("IMG");
  };
  const deleteTempImage = element => {
	delete element.AllTempImage;
  };
  const getTempImage = element => element.AllTempImage;
...
/* const attrsSrcPoster = [SRC, POSTER]; */
  ...
  const setSourcesImg = (img, settings) => {
  ...
  const setSourcesIframe = (iframe, settings) => {
	...
	setImageAttributes(iframe, settings);
  };
  ...
  const onEnter = (element, entry, settings, instance) => {
	const dontLoad = hadStartedLoading(element); /* Save status before setting it, to prevent loading it again. Fixes #526. */
...
/*	init() {
		if ( !this.preloader ) {
			console.log('vanilla_x_lazyload.js: Preloader element does not exist.');
			return;
		}
		this.hidePreloader();
	} */
